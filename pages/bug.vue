...
function submit() {
    const image = document.getElementById('fileInput') as HTMLInputElement
    const screen = image?.files?.[0] || null
    ...
    $fetch('/api/bug', {
        ...
        if (res.toString() === "Success") {
            alert('Success, your bug report has been sent to the staff team.')
        } else {
...
<template>
    ...
    <div id="body">
        ...
        <div class="home">
            ...
            <h1>Screen (falcultative) :</h1>
            <label class="uploadFile" for="fileInput">
                <div class="icon">
                    <Icon name="ic:baseline-insert-photo" class="Icon" />
                </div>
                <div class="text">
                    <span>Click to upload screen</span>
                </div>
                <input type="file" id="fileInput" accept=".png, .jpg, .jpeg" />
                <!--
        <div class="icon">
            <Icon name="material-symbols:check" class="Icon" />
        </div>
        <div class="text">
            <span>[FIleName].[extension] upload</span>
        </div>
        -->
            </label>
